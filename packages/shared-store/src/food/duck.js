...
const init = state => ({ ...initial, foods: state.foods });
...
const learning = (state, { newFoodName }) => ({
  ...setStage(state, Stages.LEARNING),
  learning: newFoodName,
});
...
const add = (state, { food }) => ({ ...state, foods: state.foods.set(food.id, food) });
